...
import { GetPlant, WeatherWateringOverview } from '@/src/types/plants.types'
...
function WeatherWateringWidget({
	data,
	isLoading
}: {
			...
			{weather ? (
				<div className='grid gap-3 sm:grid-cols-2 xl:grid-cols-4'>
					...
					<div className='rounded-2xl border border-white/8 bg-white/5 p-4'>
						...
						<p className='mt-1 text-sm text-white/50'>на ближайший полив</p>
					</div>
					...
					<div className='rounded-2xl border border-white/8 bg-white/5 p-4'>
						...
						<p className='mt-1 text-sm text-white/50'>
							{data?.upcomingPlantCount ?? 0}{' '}
							{formatCount(data?.upcomingPlantCount ?? 0, [
								'растение',
								'растения',
								'растений'
							])}{' '}
